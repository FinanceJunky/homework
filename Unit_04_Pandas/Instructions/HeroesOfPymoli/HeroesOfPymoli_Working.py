...
# purchase_data_pd.Gender.value_counts()
...
banana = item_data.groupby(["Item ID", "Item Name"]).sum()["Price"].rename("Total_Purchase_Value")
banana.head()
...
item_data.groupby(["Item ID", "Item Name"]).sum()["Price"].rename("Total Purchase Value")
